...
with open('../B08BGD4G36.csv', "r", encoding='utf8') as f:  # 打开文件
    lines = csv.reader(f)
    for line in lines:
...
with open('../B08BJJ1T9F.csv', "r", encoding='utf8') as f:  # 打开文件
    lines = csv.reader(f)
    for line in lines:
...
with open('../特征竞争性+前十+随机.csv', "r", encoding='utf8') as f:  # 打开文件
    lines = csv.reader(f)
    for line in lines:
...
number_k = 10
fp = open('Output/Time_theta.csv', "w+", encoding='utf8', newline='')
write = csv.writer(fp)
...
for n in [0]:
    ...
    for l in [3]:
        ...
        for p in range(1,11):
            ...
            time_list.append((new_time-old_time)/times)
