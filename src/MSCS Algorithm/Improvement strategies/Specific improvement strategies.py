...
content = []
with open("Data/B07756QYST_Redesign.csv", "r", encoding='utf8') as f:  # 打开文件
    lines = csv.reader(f)
    for line in lines:
        content.append([line[0] + '_Redesign'] + line[1:])

with open("Data/B07756QYST_Innovation.csv", "r", encoding='utf8') as f:  # 打开文件
    lines = csv.reader(f)
    for line in lines:
        content.append([line[0] + '_Innovation'] + line[1:])

random.seed(111)
random.shuffle(content)

fp = open('Data/B07756QYST_Strategy.csv', "w+", encoding='utf8', newline='')
write = csv.writer(fp)
...
with open('Data/B07756QYST_Strategy.csv', "r", encoding='utf8') as f:  # 打开文件
    lines = csv.reader(f)
    for line in lines:
...
#         pro_features_2.append(line[0] + '_Innovation')
...
def object_func(x):
    ...
    for i in range(len(feature_matrix[0]) - 2):
        ...
        if sum(com_x) != 0:
            object = object + 3 * sum(com_set) / (sum(com_x))
    return -object
...
review_number = len(pro_features_1) + len(pro_features_2)
k_list = [review_number]
n = -1
for number_k in [15]:
    print(review_number, number_k)
    for l in [3]:
        lam = 0.05*l
        gamma = 1
        ...
        for i in range(len(pro_features_1)):
            feature_matrix.append(importance_performance_1[i] + competitiveness_dic[pro_features_1[i].replace('_Redesign','').replace('_Innovation','')])
